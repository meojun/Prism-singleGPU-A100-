...
**따라서 전용 큐만으로는 안 된다.** 큐를 바꿔도 텐서 수명 문제는 그대로다.
어느 큐가 핸들을 나르든 상관없기 때문이다. 다음 사람은 **배선을 더 쓰기 전에
수명 문제부터 결정해라** — 타깃이 복사를 마칠 때까지 소스 텐서를 살려두거나,
아예 IPC 를 건너지 말고 host 를 경유(바운스 비용)하거나.
