...
def main():
    ...
    if not runs:
        ...
        A("")
        A("**그리고 스냅이 차이를 흡수하는 경우가 많다.** 단위테스트의 구속 "
          "시나리오에서 OFF 의 의도 `(3,3)` 과 ON 의 의도 `(3,1)` 이 **같은 그룹 "
          "`(1,3)` 으로 스냅된다** — OFF 는 GPU 3 을 포함한 그룹 중 겹침이 최대인 "
          "것을, ON 은 `{1,3}` 그룹을 고르는데 그것이 같은 그룹이다.")
        A("")
        A("이것이 이 연구의 주된 결과다. **논문 §A.2.2 의 알고리즘은 샤드를 GPU 에 "
          "자유롭게 배치할 수 있다고 전제하지만, engine pool 구현에서 배치 단위는 "
          "미리 구성된 그룹이다**(논문 §4 의 \"indivisible resource unit\"). 그룹이 "
          "서로 다른 GPU 로 구성되는 한 anti-affinity 는 아키텍처가 이미 강제하고, "
          "제약은 planner 의 의도를 기록하는 것 이상을 하지 못하는 경우가 많다. "
          "아래 표에서 arm 간 배치가 실제로 갈린 횟수가 그 답이다.")
        A("")
        if bound == 0:
