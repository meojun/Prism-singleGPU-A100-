    ...
    if not runs:
        ...
        if second_collides:
            ...
            A("논문 §A.2.2 는 충돌 시 second-lowest KVPR 로 물러나되 그 GPU 를 "
              ...
              "측정으로 확인된 셈이다.")
        elif runs and any(int(r["seed"]) for r in runs):
            A("\n`aa_second_also_collides` 는 0 이다. tp_size=2 에서는 원리상 그래야 "
              "한다 — 이미 놓인 샤드가 하나뿐이라 second-lowest 는 반드시 비충돌이다. "
              "논문 규칙과 strict 규칙이 갈리려면 tp_size>=3 이 필요하다.")
