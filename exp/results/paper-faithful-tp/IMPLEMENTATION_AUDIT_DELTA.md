...
| TP anti-affinity | NOT IMPLEMENTED | **PARTIAL** | 구현되어 라이브에서 기동하나 ON/OFF 측정을 하지 않았다. 구속 조건은 단위테스트로만 특정되어 있다. 논문 §A.2.2 를 문자 그대로 구현한 `--enable-tp-anti-affinity` 와 그보다 강한 `--enable-tp-anti-affinity-strict` 를 분리했다. |
