...
| TP anti-affinity | NOT IMPLEMENTED | **FULL** | 구현되었고 런타임에서 구속했다 — 위반 122회, 우회 71회. `step4/`, `raw/alg1_tp/`. 논문 §A.2.2 를 문자 그대로 구현한 `--enable-tp-anti-affinity` 와 그보다 강한 `--enable-tp-anti-affinity-strict` 를 분리했다. |
