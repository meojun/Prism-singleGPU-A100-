...
- **논문 알고리즘 대비**: 이 실험은 공개된 `simple_global.py` / `request_queue.py`를
  그대로 돌린 것이다. 두 레포 전 커밋을 감사한 결과 Algorithm 1(KVPR)은 목적은 같으나
  분자가 SLO·토큰 가중이 아닌 평활 요청 수인 **부분 구현**이고, Algorithm 2
  (Moore-Hodgson)는 데드라인·실행시간 추정까지만 있고 **최장 작업 제거 단계가 없어 단순
  EDF**다. 따라서 위 수치는 *공개 프로토타입의* 거동이지 논문 알고리즘의 검증이 아니다.
  구성요소별 대조는 `../STATUS_REPORT.md` §5.
